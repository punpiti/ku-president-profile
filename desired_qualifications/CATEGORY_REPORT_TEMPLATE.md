...
- มีปุ่ม `ดูภาพใหญ่`; คลิกที่กราฟไม่ควรเปิด fullscreen เอง เพราะต้องใช้คลิกเลือก node/link
- มีปุ่ม `ล้างการเลือก`
- ข้อความกำกับ node ต้องไม่หายเมื่อเปิด fullscreen หรือเมื่อมี selection; ให้จางเฉพาะกล่อง/เส้นที่ไม่เกี่ยวข้อง ไม่ลด opacity ทั้ง node group
...
ไฟล์ working copy อยู่ที่ `desired_qualifications/` แต่ไฟล์ที่ deploy จริงอยู่ที่:

`../docs/desired-qualifications/`

เมื่อต้อง deploy:

1. sync `index.html`, `categories/*.html`, `css/*.css`, `js/*.js` ไปที่ `../docs/desired-qualifications/`
2. ตรวจ `node --check` สำหรับ JS ใน `docs`
3. commit เฉพาะ `docs/desired-qualifications`
4. push ไป `main`

deploy ล่าสุดหลังปรับ Sankey flow และ fullscreen label:

- commit `8eeb0aa` - `Refine desired qualifications Sankey flow`
